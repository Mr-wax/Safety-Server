...
import { signUpValidator, signInValidator, formatZodError } from "../Validators/Validator.js";
...
export const registerUser = async (req, res) => {
  ...
  <div style="text-align: center; background: linear-gradient(135deg, #28a745, #218838); padding: 20px; border-radius: 10px 10px 0 0;">
  ...
  <div style="padding: 20px; background-color: #fff; border-radius: 0 0 10px 10px; text-align: center;">
    <p style="font-size: 16px; color: #333;">Please use the OTP below to verify your account:</p>
    <div style="font-size: 26px; font-weight: bold; color: #28a745; background-color: #f8f9fa; padding: 15px; border-radius: 8px; display: inline-block; margin: 20px auto; border: 2px dashed #28a745;">
      ${otp}
    </div>
    ...
    <p style="font-size: 16px; font-weight: bold; color: #28a745;">Safety Team</p>
  </div>
</div>
`;
    ...
    await sendMail(email, "Verify Your Account - Safety", emailTemplate);
...
export const forgotPassword = async (req, res) => {
  try {
    ...
    // Send OTP via email
    await sendMail(email, "Password Reset - Safety", resetEmailTemplate);
